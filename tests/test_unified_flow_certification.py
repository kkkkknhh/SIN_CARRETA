...
class TestUnifiedFlowCertification(unittest.TestCase):
    ...
    def _validate_pre_execution_gates(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  ⚠️  Could not freeze config (may be missing files): {freeze_err}"
                )
        ...
        if not result.get("ok", False):
            ...
            if freeze_related:
                print(
                    f"  ⚠️  Pre-execution validation warnings (freeze-related): {errors}"
                )
    ...
    def _validate_deterministic_hashes(self):
        ...
        print(
            f"  ✓ All 3 runs produced identical evidence registry hash: {hash1[:16]}..."
        )
    ...
    def _validate_post_execution_gates(self):
        ...
        if not result.get("ok", False):
            ...
            if critical_errors:
                ...
                print(
                    f"  ⚠️  Post-execution validation warnings (non-critical): {errors}"
                )
