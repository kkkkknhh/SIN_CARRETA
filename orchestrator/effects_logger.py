...
class EffectsLogger:
    ...
    def log_effect(
        self, effect_name: str, x_value: float, y_value: float, plan_id: str
    ):
        """
        ...
        self.effects_db[effect_name].append(
            {"x": x_value, "y": y_value, "plan_id": plan_id, "timestamp": timestamp}
        )

        logger.debug(
            f"Logged effect {effect_name}: x={x_value:.3f}, y={y_value:.3f}, plan={plan_id}"
        )
    ...
    def save(self):
        ...
        logger.info(
            f"Efectos guardados: {len(self.effects_db)} tipos en {self.storage_path}"
        )
    ...
    def load(self):
        ...
        logger.info(
            f"Efectos cargados: {len(self.effects_db)} tipos desde {self.storage_path}"
        )
...
class CausalEffectsManager:
    ...
    def estimate_effect(self, effect_name: str) -> "Optional[BayesLinearEffect]":
        ...
        if not self.logger.has_sufficient_data(effect_name, self.min_obs):
            ...
            logger.warning(
                f"Datos insuficientes para {effect_name}: {n_obs}/{self.min_obs} observaciones"
            )
            return None
        ...
        logger.info(
            f"Efecto {effect_name} estimado: β₁={summary['beta1_mean']:.3f} "
    ...
    def get_all_effects(self) -> Dict[str, Dict]:
        ...
        """
        return {name: model.get_summary() for name, model in self.estimators.items()}
...
def periodic_causal_analysis(effects_logger: EffectsLogger, min_obs: int = 30) -> Dict:
    ...
    for effect, summary in results.items():
        if summary.get("fitted"):
            print(f"\n{effect}:")
            print(
                f"  β₁ (efecto): {summary['beta1_mean']:.3f} "
                f"{summary['beta1_interval_95']}"
            )
