...
if __name__ == "__main__":
    ...
    print(
        f"Loaded DECALOGO_INDUSTRIAL template version: {template.get('version', 'unknown')}"
    )
...
def load_dnp_standards(path: Optional[str] = None) -> Dict[str, Any]:
    ...
    standards_path = (
        Path(path) if path else get_dnp_path(os.getenv("DNP_PATH_OVERRIDE"))
    )
    ...
    return {"metadata": {"version": "2.0.0"}, "decalogo_dimension_mapping": {}}
...
def ensure_aligned_templates() -> Dict[str, Any]:
    ...
    logger.info(
        "Loaded templates: %s questions, %s dimension mappings",
        decalogo_questions,
        dnp_dimensions,
    )
...
def get_question_by_id(
    question_id: str, decalogo_industrial: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
...
def get_dimension_weight(
    point_code: str, dimension: str, dnp_standards: Optional[Dict[str, Any]] = None
) -> float:
    """
...
if __name__ == "__main__":
    ...
    if questions:
        ...
        for q in questions[:3]:
            print(
                f"- {q.get('id')}: {q.get('point_title')} (Dimension {q.get('dimension')})"
            )
