...
class MonetaryMatch:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context_excerpt": self.context[:100] + "..."
...
class MonetaryDetector:
    ...
    def _trace_state(self, event: str) -> None:
        registry_type = (
            type(self.evidence_registry).__name__ if self.evidence_registry else "None"
        )
        self.logger.info(
    ...
    def _compile_patterns() -> Dict[str, re.Pattern]:
        ...
        timeframe_patterns = [
            (
                r"(?i)(?:anual(?:es)?|por año|al año|cada año)",
                FinancialTimeframe.ANNUAL,
            ),
            (
                r"(?i)(?:cuatrienio|multianual|plurianual|plan plurianual)",
                FinancialTimeframe.MULTIANNUAL,
            ),
        ...
        category_patterns = [
            (
                r"(?i)(?:presupuesto(?:s)?|recursos financieros)",
                MonetaryCategory.BUDGET,
            ),
            ...
            (
                r"(?i)(?:donaci[oó]n(?:es)?|aporte(?:s)?|cooperaci[oó]n internacional)",
                MonetaryCategory.GRANT,
            ),
            (
                r"(?i)(?:pr[eé]stamo(?:s)?|cr[eé]dito(?:s)?|financiaci[oó]n)",
                MonetaryCategory.LOAN,
            ),
        ]
        ...
        compiled_timeframe_patterns = [
            (re.compile(p), t) for p, t in timeframe_patterns
        ]
        ...
        for pattern_name, pattern in self.patterns.items():
            for match in pattern.finditer(text):
                ...
                category = self._classify_category(
                    match.group(), text[max(0, match.start() - 100): match.end() + 100]
                )
                ...
                monetary_match = {
                    ...
                    if isinstance(category, MonetaryCategory)
                    else category,
                    "timeframe": timeframe.value
                    if isinstance(timeframe, FinancialTimeframe)
                    else timeframe,
    ...
    def _parse_amount(text: str) -> float:
        ...
        if dollar_match:
            ...
            amount_str = amount_str.replace(
                ",", "."
            )  # Convert remaining commas to decimal points
            try:
...
if __name__ == "__main__":
    ...
    for detection in detections:
        print(
            f"- {detection['original_text']}: {detection['amount']} {detection['currency']} "
