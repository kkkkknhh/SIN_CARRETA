...
def _power_approx(p: float, n: int, alpha: float = 0.05) -> float:
    if not SCIPY:
        return float(min(1.0, max(0.0, n * abs(p - 0.5))))
...
class DAGValidator:
    ...
    def calculate_acyclicity_pvalue_advanced(
        self, plan_name: str, iterations: int = 2000
    ) -> Dict[str, Any]:
        t0 = time.time()
        ...
        post = (p * 0.5) / (
            p * 0.5 + (1 - p) * 0.5
        )  # update bayesiana simple con prior 0.5
