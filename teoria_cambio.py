...
def assert_policy(pid: str) -> None:
    if not _P_PAT.match(pid):
        raise ValueError(f"Policy ID inválido (P1..P10): {pid}")


def assert_dim(did: str) -> None:
    if not _D_PAT.match(did):
        raise ValueError(f"Dimension ID inválido (D1..D6): {did}")


def assert_q(qid: str) -> None:
    if not _Q_PAT.match(qid):
        raise ValueError(f"Question ID inválido (Q1..): {qid}")
...
def to_rubric_key(question_uid: str) -> str:
    ...
    if not (m_dim and m_q):
        raise ValueError(f"UID no estandarizable: {question_uid}")
...
class EmbeddingBackend:
    def encode(self, _texts: Sequence[str]) -> np.ndarray:
...
class TfidfBackend(EmbeddingBackend):
    def __init__(self):
        ...
        self.vec.fit(
            list(seed)
            + [
                "educación salud empleo vivienda agua infancia seguridad ambiente género víctimas indicadores base"
            ]
        )
...
def build_embedding_backend() -> EmbeddingBackend:
    try:
        ...
        log.info("Embeddings: SentenceTransformer")
        return SentenceTransformerBackend()
    except Exception:
        log.info("Embeddings: TF-IDF fallback")
...
class CausalExtractor:  # iface
    def extract(self, _text_block: str) -> List[CausalElement]:
        raise NotImplementedError
            ...
            for t in sent:
                if t.pos_ == "VERB" and t.lemma_.lower() in CAUSAL_VERBS:
                    ...
                    for ch in t.children:
                        if ch.dep_ in ("obj", "dobj", "xcomp", "ccomp", "attr"):
                            effect = ch.subtree.text
                            break
                    if not effect:
                        effect = sent.text
                    out.append(
...
class RegexExtractor(CausalExtractor):
    _PAT = re.compile(
        r"\b(?:generar|producir|crear|entregar|lograr|alcanzar|conseguir|mejorar|reducir|aumentar|impactar|transformar|cambiar|afectar)\b\s+([^\.;]+)",
        re.IGNORECASE,
    )
...
def build_extractor() -> CausalExtractor:
    try:
        ...
        log.info("Extractor: regex fallback")
...
def classify_by_verb(v: str) -> str:
    if v in {"generar", "producir", "crear", "entregar"}:
        return "output"
    if v in {"lograr", "alcanzar", "conseguir", "mejorar", "reducir", "aumentar"}:
        return "outcome"
    if v in {"impactar", "transformar", "cambiar", "afectar"}:
        return "impact"
    return "output"
...
class TeoriaCambioIndustrial:
    ...
    def segment_text_by_policy(self, text: str) -> Dict[str, List[str]]:
        ...
        active = {
            p for p, col in zip(keys, sims.T) if col.max() >= self.relevance_threshold
        }
        ...
        for sent, idx in zip(sentences, assigns):
            pol = keys[idx]
            if pol in active:
                segs.setdefault(pol, []).append(sent)
    ...
    def _extract_for_policy(self, block: str, policy: str) -> List[CausalElement]:
        ...
        for e in elems:
            e.policy_area = policy
        self._cache.set(key, elems)
        return elems

    def _score_block(
        self, policy: str, elems: List[CausalElement]
    ) -> List[EvidenceEntry]:
        evs: List[EvidenceEntry] = []
    ...
    def verificar_marco_logico_completo(self, text_or_segments: Any) -> Dict[str, Any]:
        ...
        summary = {
            "success": True,
            "counts": {"policies": len(segments), "evidences": len(all_evidence)},
            "metrics": metrics_payload,
        }
        immut = {
            "version": "5.0.0",
            "timestamp_utc": datetime.now(timezone.utc).isoformat(),
            "hash": DeterministicHasher.sha256(
                {"toc_graph": toc_graph, "summary": summary}
            ),
        }
