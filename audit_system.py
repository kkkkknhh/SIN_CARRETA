...
class MINIMINIMOONAuditor:
    ...
    def __init__(self, config_dir: Path, test_plan_path: Path):
        ...
        if not self.orchestrator_source.exists():
            raise FileNotFoundError(
                f"Orchestrator not found: {self.orchestrator_source}"
            )

    def audit_stage_execution(
        self, pipeline_results: Dict[str, Any]
    ) -> List[StageAuditResult]:
        ...
        for result in results:
            ...
            self.logger.info(
                f"  {status} Stage {result.stage_number}: {result.stage_name}"
            )
            if result.error:
    ...
    def audit_module_integration(self) -> List[ModuleIntegrationResult]:
        ...
        for result in results:
            status = (
                "✓"
                if (result.found_in_codebase and result.imported_in_orchestrator)
                else "✗"
            )
            self.logger.info(
                f"  {status} {result.module_name} (stage {result.expected_stage})"
            )
            if result.issues:
    ...
    def _check_module_integration(
        ...
        if not found_in_codebase:
            issues.append(
                f"Module file not found in codebase (searched: {', '.join(str(p) for p in potential_files)})"
            )
    ...
    def audit_evidence_coverage(
        ...
        answers_report = pipeline_results.get("evaluations", {}).get(
            "answers_report", {}
        )
        ...
        coverage_percentage = (
            (questions_with_3plus / 300 * 100) if total_found > 0 else 0.0
        )
        ...
        self.logger.info(
            f"Questions with doctoral justification: {questions_with_doctoral}"
        )
        self.logger.info(f"Coverage: {coverage_percentage:.1f}%")

        status = "✓" if meets_requirements else "✗"
        self.logger.info(
            f"{status} Coverage requirement (300 questions with 3+ sources): {meets_requirements}"
        )
    ...
    def audit_determinism(self) -> DeterminismResult:
        ...
        try:
            ...
            results1 = orchestrator1.process_plan_deterministic(
                str(self.test_plan_path)
            )

            self.logger.info("Running orchestrator - Run 2...")
            orchestrator2 = CanonicalDeterministicOrchestrator(
                config_dir=self.config_dir, enable_validation=True, log_level="WARNING"
            )
            results2 = orchestrator2.process_plan_deterministic(
                str(self.test_plan_path)
            )
            ...
            # Compare scores
            answers1 = (
                results1.get("evaluations", {})
                .get("answers_report", {})
                .get("question_answers", [])
            )
            answers2 = (
                results2.get("evaluations", {})
                .get("answers_report", {})
                .get("question_answers", [])
            )
            ...
            run2_scores = {
                qa["question_id"]: qa.get("raw_score", 0.0) for qa in answers2
            }
            ...
            if not is_deterministic:
                ...
                if score_mismatches:
                    self.logger.error(
                        f"  {len(score_mismatches)} score mismatches found!"
                    )
    ...
    def run_full_audit(self) -> AuditReport:
        ...
        try:
            ...
            pipeline_results = orchestrator.process_plan_deterministic(
                str(self.test_plan_path)
            )
            orchestrator_version = pipeline_results.get(
        ...
        modules_ok = all(
            r.found_in_codebase and r.imported_in_orchestrator for r in module_audit
        )
        ...
        summary = {
            ...
            "modules_imported": sum(
                1 for r in module_audit if r.imported_in_orchestrator
            ),
...
def main():
    ...
    parser.add_argument(
        "--plan", type=Path, required=True, help="Test plan file (PDF or text)"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    try:
        auditor = MINIMINIMOONAuditor(config_dir=args.config, test_plan_path=args.plan)
