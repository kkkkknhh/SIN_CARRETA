        ...
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Name) and node.func.id == "PlanProcessor":
                ...
                if len(node.args) > 0:
                    ...
                    config_dir_found = any(
                        kw.arg == "config_dir" for kw in node.keywords
                    )
                    assert config_dir_found, "config_dir keyword argument not found"
                    print(
                        f"    ✓ PlanProcessor initialized with config_dir keyword argument"
                    )
                    found_call = True
...
def test_questionnaire_engine_initialization():
    ...
    for node in ast.walk(tree):
        if (
            ...
            print(
                f"    ✓ QuestionnaireEngine initialized with keyword args: {', '.join(keyword_args)}"
            )
            found_call = True
