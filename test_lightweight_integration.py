...
def check_method_exists_in_file(filepath: Path, class_name: str, method_name: str) -> bool:
...
def test_evaluate_from_evidence():
    ...
    if not has_method:
        logger.error("  ✗ Method evaluate_from_evidence() not found in ExtractorEvidenciaIndustrialAvanzado")
        return False
    ...
    try:
        ...
        if "@dataclass" in content and "class EvaluationResult" in content:
            ...
            if "metadata:" in content or "metadata =" in content:
                ...
                logger.warning("  ⚠ metadata field not clearly found in EvaluationResult")
...
def test_doctoral_argumentation_integration():
    ...
    try:
        ...
        if all_found:
            ...
            logger.info("⚠️  TEST 4 PARTIAL: Some components missing but core integration present")
...
def test_integration_completeness():
    ...
    checklist = [
        ("module_contribution_mapper.py exists", Path("module_contribution_mapper.py").exists()),
        ("evaluate_from_evidence() in Decatalogo_principal.py",
         check_method_exists_in_file(Path("Decatalogo_principal.py"), "ExtractorEvidenciaIndustrialAvanzado", "evaluate_from_evidence")),
        ("_synthesize_multi_source_evidence() in questionnaire_engine.py",
         check_method_exists_in_file(Path("questionnaire_engine.py"), "QuestionnaireEngine", "_synthesize_multi_source_evidence")),
        ("Doctoral integration in answer_assembler.py",
         check_import_in_method(Path("answer_assembler.py"), "assemble", "DoctoralArgumentationEngine")),
    ]
