...
class FileLocationValidator:
    ...
    def __init__(self):
        ...
        from repo_paths import REPO_ROOT, get_decalogo_path, get_dnp_path
    ...
    def validate_module_imports(self) -> bool:
        ...
        try:
            ...
            if not industrial or "questions" not in industrial:
                self.errors.append(
                    "❌ decalogo_loader.get_decalogo_industrial() failed"
                )
                return False
            print(
                f"✓ decalogo_loader.get_decalogo_industrial() works ({len(industrial['questions'])} questions)"
            )
        ...
        try:
            ...
            for key, rel_path in paths_config.items():
                ...
                if not resolved.exists():
                    self.errors.append(
                        f"❌ Config path '{key}' resolves to non-existent file: {resolved}"
                    )
                    return False
    ...
    def run_validation(self) -> bool:
        ...
        print("-" * 80)
        exists_decalogo = self.validate_file_exists(
            self.decalogo_path, "decalogo-industrial"
        )
        ...
        print("-" * 80)
        _valid_decalogo = self.validate_json_structure(
            self.decalogo_path, "decalogo-industrial"
        )
