...
class ConnectionStabilityAnalyzer:
    ...
    def get_or_create_metrics(self, connection_id: str) -> ConnectionMetrics:
        ...
        if connection_id not in self.connection_metrics:
            self.connection_metrics[connection_id] = ConnectionMetrics(connection_id=connection_id)
    ...
    def validate_interface(self, source: str, target: str, data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        ...
        for field_name, expected_type in flow_spec.output_schema.items():
            ...
            if not self._validate_type(actual_value, expected_type):
                errors.append(f"Type mismatch for {field_name}: expected {expected_type.value}, got {type(actual_value).__name__}")
                self.get_or_create_metrics(connection_id).type_incompatibilities += 1
        
        if errors:
            self.capture_schema_mismatch(
                connection_id=connection_id,
                expected_schema={k: v.value for k, v in flow_spec.output_schema.items()},
    ...
    def verify_cardinality(self, connection_id: str, actual_count: int) -> Tuple[bool, str]:
        """
        ...
        flow_spec = None
        for spec in self.flow_specifications.values():
            if spec.flow_id == connection_id or f"{spec.source}_to_{spec.target}" == connection_id:
                flow_spec = spec
                break
    ...
    def analyze_connection_stability(self, connection_id: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "retry_rate": metrics.retry_count / metrics.total_attempts if metrics.total_attempts > 0 else 0.0,
    ...
    def _identify_stability_issues(self, metrics: ConnectionMetrics) -> List[str]:
        ...
        retry_rate = metrics.retry_count / metrics.total_attempts if metrics.total_attempts > 0 else 0.0
        if retry_rate > self.retry_rate_threshold:
            issues.append(f"High retry rate: {retry_rate:.2%} > {self.retry_rate_threshold:.2%}")
        
        if metrics.max_backoff_delay > self.max_backoff_threshold_ms:
            issues.append(f"Excessive backoff delay: {metrics.max_backoff_delay:.0f}ms > {self.max_backoff_threshold_ms:.0f}ms")
    ...
    def generate_verdict(self, connection_id: str) -> ConnectionVerdict:
        ...
        if metrics.type_incompatibilities > 0:
            violations.append(f"{metrics.type_incompatibilities} type incompatibilities detected")
        if metrics.cardinality_violations > 0:
            violations.append(f"{metrics.cardinality_violations} cardinality violations detected")
        violations.extend(stability_issues)
        
        recommendations = []
        if metrics.error_rate > self.error_rate_threshold:
            recommendations.append("Implement circuit breaker pattern for fault tolerance")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        total_schema_mismatches = sum(len(mismatches) for mismatches in self.schema_mismatches.values())
        ...
        report = {
            "summary": {
                ...
                "stability_rate": stable_connections / total_connections if total_connections > 0 else 0.0,
                "suitability_rate": suitable_connections / total_connections if total_connections > 0 else 0.0
            },
...
def create_connection_stability_analyzer(flow_doc_path: str = "tools/flow_doc.json") -> ConnectionStabilityAnalyzer:
