...
class Gate1CanonicalOrderTest(unittest.TestCase):
    ...
    def test_runtime_order_parsing(self):
        ...
        self.assertIn(
            "order",
            flow_runtime,
            "flow_runtime.json must contain 'order' key"
        )
        ...
        self.assertIsInstance(
            runtime_order,
            list,
            "runtime order must be a list"
        )
        self.assertGreater(
            len(runtime_order),
            0,
            "runtime order must not be empty"
        )
    ...
    def test_exact_order_match(self):
        ...
        for i, (canonical_node, runtime_node) in enumerate(zip(canonical_order, runtime_order)):
            self.assertEqual(
                canonical_node,
                runtime_node,
        ...
        print(f"✅ GATE #1 PASSED: Canonical order ({len(canonical_order)} nodes) matches runtime execution exactly")
...
class Gate2SystemValidatorsTest(unittest.TestCase):
    ...
    def test_validate_post_execution_method_exists(self):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "validate_post_execution":
                method_found = True
                break
    ...
    def test_subprocess_invocation_to_rubric_check(self):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if (hasattr(node.func, 'attr') and node.func.attr == 'run' and
                    hasattr(node.func.value, 'id') and node.func.value.id == 'subprocess'):
        ...
        print(f"✅ Found {len(subprocess_calls)} subprocess.run() call(s) in system_validators.py")
    ...
    def test_exit_code_handling(self):
        ...
        self.assertTrue(
            exit_code_found,
            "Must have exit code handling for subprocess.run() result"
        )
        ...
        specific_codes_documented = (
            "exit code 0" in source or "returncode == 0" in source or
            "exit code 2" in source or "returncode == 2" in source or
            "exit code 3" in source or "returncode == 3" in source
        )
    ...
    def test_correct_arguments_passed(self):
        ...
        self.assertIn(
            "answers_path",
            source,
            "Must pass answers_path to rubric_check.py"
        )
        self.assertIn(
            "rubric_path",
            source,
            "Must pass rubric_path to rubric_check.py"
        )
...
class Gate3UnifiedPipelineTest(unittest.TestCase):
    ...
    def test_system_validator_initialization(self):
        ...
        self.assertIn(
            ...
            source,
            "Must initialize SystemHealthValidator"
        )
    ...
    def test_post_execution_validation_call(self):
        ...
        self.assertTrue(
            re.search(r'validate_post_execution\([^)]*results', source) or
            re.search(r'validate_post_execution\([^)]*complete_results', source),
    ...
    def test_artifacts_packaging_in_final_bundle(self):
        ...
        for artifact in required_artifacts:
            self.assertIn(
                artifact,
                source,
                f"Final bundle must include '{artifact}' artifact"
            )
        
        print(f"✅ GATE #3 PASSED: All {len(required_artifacts)} required artifacts packaged")
...
class Gate4AnswerAssemblerTest(unittest.TestCase):
    ...
    def test_rubric_scoring_has_weights_section(self):
        ...
        self.assertGreater(
            len(questions),
            0,
            "'questions' section must not be empty"
        )
    ...
    def test_weight_loading_in_init(self):
        ...
        weight_pattern_found = any(re.search(pattern, source) for pattern in weight_patterns)
        self.assertTrue(
            weight_pattern_found,
            "Must have weight loading mechanism"
        )
        
        print("✅ Weight loading from RUBRIC_SCORING.json verified in AnswerAssembler.__init__()")
    ...
    def test_export_answers_report_method(self):
        ...
        self.assertTrue(
            re.search(r'def\s+assemble\(', source),
            "Must have assemble() method"
        )
    ...
    def test_answers_report_schema(self):
        ...
        self.assertIn(
            "summary",
            answers_report,
            "answers_report.json must contain 'summary' key"
        )
        ...
        self.assertIn(
            "answers",
            answers_report,
            "answers_report.json must contain 'answers' key"
        )
        ...
        if len(answers) > 0:
            ...
            for field in required_fields:
                self.assertIn(
                    field,
                    sample_answer,
                    f"Answer must contain '{field}' field"
                )
        ...
        self.assertIn(
            "total_questions",
            summary,
            "summary must contain 'total_questions'"
        )
        
        print(f"✅ GATE #4 PASSED: answers_report.json schema validated ({len(answers)} answers)")
...
class Gate5CIWorkflowTest(unittest.TestCase):
    ...
    def test_ci_workflow_exists(self):
        ...
        self.assertTrue(
            self.ci_yml_path.exists(),
            f"CI workflow missing: {self.ci_yml_path}"
        )
    ...
    def test_rubric_validation_job_exists(self):
        ...
        self.assertIn(
            "rubric-validation",
            jobs,
            "ci.yml must contain 'rubric-validation' job"
        )
    ...
    def test_deterministic_pipeline_validation_job_exists(self):
        ...
        for pattern in expected_step_patterns:
            ...
            for step in steps:
                ...
                step_run = step.get("run", "")
                if re.search(pattern, step_name, re.IGNORECASE) or re.search(pattern, step_run, re.IGNORECASE):
                    pattern_found = True
                    break
        ...
        print("✅ deterministic-pipeline-validation job verified (freeze, pre-exec, triple-run, post-exec)")
...
class IntegrationTestSuite(unittest.TestCase):
    ...
    def test_full_pipeline_integration(self):
        ...
        suite.addTests(unittest.TestLoader().loadTestsFromTestCase(Gate1CanonicalOrderTest))
        
        # Gate #2: System Validators
        suite.addTests(unittest.TestLoader().loadTestsFromTestCase(Gate2SystemValidatorsTest))
        
        # Gate #3: Unified Pipeline
        suite.addTests(unittest.TestLoader().loadTestsFromTestCase(Gate3UnifiedPipelineTest))
        
        # Gate #4: Answer Assembler
        suite.addTests(unittest.TestLoader().loadTestsFromTestCase(Gate4AnswerAssemblerTest))
