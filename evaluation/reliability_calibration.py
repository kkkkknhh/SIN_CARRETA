...
def reliability_weighted_score(
    raw_score: float, calibrator: ReliabilityCalibrator, metric: str = "precision"
) -> float:
    """
...
class CalibratorManager:
    ...
    def update_from_ground_truth(
        self, detector_name: str, y_true: np.ndarray, y_pred: np.ndarray
    ):
    ...
    def get_all_stats(self) -> Dict:
        ...
        return {name: cal.get_stats() for name, cal in self.calibrators.items()}
...
def calibration_workflow(
    manager: CalibratorManager, labeled_data: Dict[str, List[Tuple[int, int]]]
):
    """
    ...
    for detector, stat in stats.items():
        print(f"\n{detector}:")
        print(
            f"  Precisión: {stat['precision']['mean']:.2%} "
            f"{stat['precision']['interval']}"
        )
        print(f"  Recall: {stat['recall']['mean']:.2%} {stat['recall']['interval']}")
