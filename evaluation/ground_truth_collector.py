...
try:
    ...
    logger.warning(
        "pandas not available. Ground truth collector will use JSON fallback."
    )
...
class GroundTruthCollector:
    ...
    def export_for_labeling(self, output_file: Path):
        ...
        if PANDAS_AVAILABLE and output_file.suffix == ".csv":
            ...
            logger.info(
                "%s items exportados a JSON: %s", len(self.pending_labels), json_file
            )
    ...
    def import_labeled_data(input_file: Path) -> Dict[str, List[Tuple]]:
        ...
        if PANDAS_AVAILABLE and input_file.suffix == ".csv":
            ...
            for detector in df["detector"].unique():
                ...
                pairs = list(
                    zip(
                        subset["ground_truth"].astype(int),
            ...
            for item in labeled:
                ...
                if detector not in grouped:
                    grouped[detector] = []
                grouped[detector].append(
                    (int(item["ground_truth"]), int(item["prediction"]))
                )

            logger.info(
                "Importados %s labels de %s detectores desde JSON",
                len(labeled),
                len(grouped),
            )
            return grouped
    ...
    def load_checkpoint(self, checkpoint_file: Path = None):
        ...
        if checkpoint_file.exists():
            ...
            logger.info(
                "Checkpoint cargado: %s (%s items)",
                checkpoint_file,
                len(self.pending_labels),
            )
        else:
...
def create_ground_truth_collector(
    detector_name: str, base_path: Path = None
