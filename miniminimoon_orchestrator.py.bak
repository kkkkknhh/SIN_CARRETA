...
class CanonicalDeterministicOrchestrator:
    ...
    def warmup_models(self):
        ...
        try:
            ...
            # Dry-run each if per-question method exists
    ...
    def _encode_segments_dynamic(self, segment_texts: List[str]) -> List[Any]:
        ...
        while idx < len(segment_texts):
            # If large backlog, escalate to 64
            if remaining >= 64:
                batch_size = 64
            else:
    ...
    def _parallel_questionnaire_evaluation(self) -> Dict[str, Any]:
        ...
        if not question_ids and hasattr(engine, "questions"):
            ...
            if isinstance(q_attr, dict):
                ...
                question_ids = q_attr
        # If engine cannot expose question ids or method evaluate_question missing → fallback
        ...
        # Build consistent structure similar to sequential engine (approximation)
        aggregate = {
    ...
    def process_plan_deterministic(self, plan_path: str) -> Dict[str, Any]:
        ...
        cached_embeddings = self.intermediate_cache.get(cache_key_embeddings)
        if cached_embeddings:
            self.logger.info("Embeddings cache HIT")
            embeddings = self._run_stage(
                PipelineStage.EMBEDDING,
                lambda: cached_embeddings,
                results["stages_completed"]
            )
        else:
            embeddings = self._run_stage(
        ...
        cached_resp = self.intermediate_cache.get(cache_key_resp)
        if cached_resp:
        ...
        # Flow #14: Questionnaire (parallel)
...
def verify_reproducibility(
    ...
    if evidence_ok and flow_ok:
        print("# filepath: /Users/recovered/Music/MINIMINIMOON-main/miniminimoon_orchestrator.py
# coding=utf-8
"""
CANONICAL DETERMINISTIC ORCHESTRATOR — FINAL VERSION (with caching & pooling enhancements)
=========================================================================================
