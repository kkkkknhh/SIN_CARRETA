...
class SpacyModelLoader:
    ...
    def load_model(
        self, model_name: Optional[str] = None, download_if_missing: bool = True
    ) -> Any:
        """
                ...
                if self.auto_download and download_if_missing:
                    ...
                    for _ in range(self.max_retries):
                        try:
                            ...
                            logger.warning(
                                "Download attempt failed for %s: %s", model_to_load, e
                            )
                            continue
                else:
                    logger.warning(
                        "Model %s not found and download is disabled", model_to_load
                    )
            except Exception as e:
...
def create_safe_processor(
    model_name: Optional[str] = None, auto_download: bool = True
) -> SafeSpacyProcessor:
    """
