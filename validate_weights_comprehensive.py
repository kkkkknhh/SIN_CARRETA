...
def validate_rubric_weights(rubric_file="rubric_scoring.json"):
    ...
    print(
        ...
        if has_questions
        else f"Has 'questions': {has_questions} ✗"
    )
    print(
        f"Has 'weights': {has_weights} ✓"
        if has_weights
        else f"Has 'weights': {has_weights} ✗"
    )
    ...
    print(
        f"All 300 weights present: {len(weight_ids) == 300 and weight_ids == expected_ids} ✓"
        if len(weight_ids) == 300 and weight_ids == expected_ids
        else "All 300 weights present: False ✗"
    )
    ...
    all_correct = (
        ...
        and abs(list(unique_weights)[0] - expected_weight) < 1e-15
    )
    print(
        f"All weights = 0.0033333333333333335: {all_correct} ✓"
        if all_correct
        else "All weights uniform: False ✗"
    )
    ...
    print(
        f"Within tolerance (<1e-10): {within_tolerance} ✓"
        if within_tolerance
        else "Within tolerance: False ✗"
    )
    ...
    print(
        f"Questions section contains base 30 questions: {questions_match_base} ✓"
        if questions_match_base
        else "Questions structure differs ℹ"
    )
