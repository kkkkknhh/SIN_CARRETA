...
class PipelineOrchestrator:
    ...
    def _load_components(self):
        ...
        try:
            self.components["causal_pattern_detector"] = (
                create_causal_pattern_detector()
            )
    ...
    def _analyze_component_coverage(self):
        ...
        coverage_pct = (
            (covered_count / total_questions) * 100 if total_questions > 0 else 0
        )

        logger.info(
            "DECALOGO question coverage: %s/%s questions (%.1f%%)",
            covered_count,
            total_questions,
            coverage_pct,
        )
    ...
    def extract_evidence(
        self, plan_text: str, _plan_id: str
        ...
        if "teoria_cambio" in self.components:
            try:
                ...
                evidence_item_q4 = EvidenceItem(
                    ...
                    confidence=0.7,
                    metadata={
                        "logical_linkage": tc_results["de1_q4"]["answer"] == "Sí"
                    },
                )
                ...
                evidence_item_q6 = EvidenceItem(
                    ...
                    confidence=0.7,
                    metadata={
                        "framework_complete": tc_results["de1_q6"]["answer"] == "Sí"
                    },
                )
        ...
        if "causal_pattern_detector" in self.components:
            try:
                ...
                evidence_item_q4 = EvidenceItem(
                    ...
                    metadata={
                        "has_logical_linkage": decalogo_analysis["de1_q4"]["answer"]
                        == "Sí"
                    },
                )
                evidence_by_question["DE-1 Q4"].append(evidence_item_q4)
                ...
                evidence_item_q6 = EvidenceItem(
                    ...
                    metadata={
                        "has_complete_framework": decalogo_analysis["de1_q6"]["answer"]
                        == "Sí"
                    },
                )
                evidence_by_question["DE-1 Q6"].append(evidence_item_q6)
                ...
                logger.error(
                    "Error collecting evidence from causal_pattern_detector: %s", e
                )
        ...
        if "feasibility_scorer" in self.components:
            try:
                ...
                evidence_item_q3 = EvidenceItem(
                    ...
                    metadata={
                        "have_baselines_targets": de1_q3_result["answer"] == "Sí"
                    },
                )
        ...
        if "responsibility_detector" in self.components:
            try:
                ...
                responsible_count = len(
                    [e for e in responsibility_entities if e.confidence >= 0.6]
                )
                ...
                logger.error(
                    "Error collecting evidence from responsibility_detector: %s", e
                )
    ...
    def evaluate_plan(
        self, plan_text: str, plan_id: str
    ) -> Dict[str, DecalogoEvaluation]:
        """
        ...
        for question_id, question in self.questions.items():
            ...
            if not question_evidence:
                ...
                logger.warning(
                    "No evidence available for %s: %s", question_id, question.text
                )
                continue
    ...
    def _evaluate_question(
        question: DecalogoQuestion, evidence: List[EvidenceItem]
    ) -> DecalogoEvaluation:
        """
        ...
        if question.full_id == "DE-1 Q4":
            ...
            has_linkage = any(
                item.metadata.get("logical_linkage", False)
                or item.metadata.get("has_logical_linkage", False)
                for item in evidence
            )

            linkage_scores = [
                item.content.get("overall_score", 0.0)
            ...
            avg_score = (
                sum(linkage_scores) / len(linkage_scores) if linkage_scores else 0.0
            )
            ...
            has_framework = any(
                item.metadata.get("framework_complete", False)
                or item.metadata.get("has_complete_framework", False)
                for item in evidence
            )

            framework_scores = [
                item.content.get("completeness_score", 0.0)
                if isinstance(item.content, dict)
                else 0.0
                for item in evidence
            ]
            ...
            positive_answers = [
                item for item in evidence if item.content.get("answer") == "Sí"
            ]
            ...
            score = (
                sum(item.content.get("coverage_ratio", 0.0) for item in evidence)
                / len(evidence)
                if evidence
                else 0.0
            )
            ...
            entity_counts = [item.metadata.get("entity_count", 0) for item in evidence]
            ...
            resources_adequate = any(
                item.metadata.get("resources_adequate", False) for item in evidence
            )
            ...
            avg_score = (
                sum(adequacy_scores) / len(adequacy_scores) if adequacy_scores else 0.0
            )
            ...
            confidence = avg_confidence
            justification = (
                f"Evaluación basada en {len(evidence)} fuentes de evidencia."
            )
    ...
    def generate_evaluation_report(
        self, evaluations: Dict[str, DecalogoEvaluation]
    ) -> Dict[str, Any]:
        """
        ...
        for dimension_id, dimension in dimensions.items():
            ...
            if questions:
                dimension["score"] = sum(q["score"] for q in questions.values()) / len(
                    questions
                )
            else:
                dimension["score"] = 0.0
        ...
        summary = {
            ...
            "answered_questions": sum(
                d["answered_questions"] for d in dimensions.values()
            ),
            "total_questions": len(self.questions),
        }
        ...
        report = {
            ...
            "evaluation_coverage": summary["answered_questions"]
