...
class IntegratedEvaluationSystem:
    ...
    def evaluate_pdm_complete(
        ...
        integrated_results = {
            ...
            "questionnaire_300": {
                ...
                "total_questions_evaluated": len(
                    questionnaire_results.get("evaluation_matrix", [])
                ),
            },
            # Resumen Ejecutivo Combinado
            "executive_summary": self._generate_executive_summary(
                orchestrator_results, questionnaire_results
            ),
        }
        ...
        # ========================================
        if export_json:
            output_path = self._export_results(integrated_results, pdm_path, output_dir)
    ...
    def _log_orchestrator_summary(results: Dict[str, Any]):
        ...
        logger.info(
            "    • Responsabilidades detectadas: %s",
            len(results.get("responsibilities", [])),
        )
        logger.info(
            "    • Contradicciones detectadas: %s",
            results.get("contradictions", {}).get("total", 0),
        )
        logger.info(
            "    • Valores monetarios detectados: %s", len(results.get("monetary", []))
        )
        logger.info(
            "    • Patrones causales detectados: %s",
            len(results.get("causal_patterns", [])),
        )
        logger.info(
            "    • Teoría de cambio válida: %s",
            results.get("teoria_cambio", {}).get("is_valid", False),
        )
    ...
    def _log_questionnaire_summary(results: Dict[str, Any]):
        ...
        logger.info(
            "    • Total preguntas evaluadas: %s",
            len(results.get("evaluation_matrix", [])),
        )
        logger.info("    • Score global: %.1f%%", summary.get("score_percentage", 0))
        logger.info("    • Clasificación: %s", summary.get("classification", "N/A"))
        logger.info(
            "    • Puntos temáticos evaluados: %s", summary.get("points_evaluated", 0)
        )
    ...
    def _generate_executive_summary(
        ...
        q_score = questionnaire_results.get("global_summary", {}).get(
            "score_percentage", 0
        )

        # Score del MINIMINIMOON Decálogo (0-100)
        m_score = (
            orchestrator_results.get("evaluation", {}).get("global_score", 0) * 100
        )

        # Score combinado (promedio ponderado)
        combined_score = (q_score * 0.6) + (
            m_score * 0.4
        )  # 60% questionnaire, 40% miniminimoon
        ...
        if combined_score >= 85:
            ...
            recommendation = (
                "PDM con vacíos críticos. Reformulación sustancial requerida."
            )
        else:
            classification = "DEFICIENTE"
            recommendation = (
                "PDM no cumple estándares básicos. Rediseño completo necesario."
            )

        # Identificar fortalezas y debilidades
        dim_scores = questionnaire_results.get("dimension_summary", {})
        strongest_dim = (
            max(dim_scores.items(), key=lambda x: x[1]) if dim_scores else ("N/A", 0)
        )
        weakest_dim = (
            min(dim_scores.items(), key=lambda x: x[1]) if dim_scores else ("N/A", 0)
        )
        ...
        return {
            ...
            "key_findings": {
                ...
                "contradictions_detected": orchestrator_results.get(
                    "contradictions", {}
                ).get("total", 0),
                "budget_defined": len(orchestrator_results.get("monetary", [])) > 0,
                "theory_of_change_valid": orchestrator_results.get(
                    "teoria_cambio", {}
                ).get("is_valid", False),
            },
            "priority_actions": self._generate_priority_actions(
                orchestrator_results, questionnaire_results, weakest_dim[0]
            ),
        }
    ...
    def _generate_priority_actions(
        ...
        if contradictions.get("total", 0) > 5:
            actions.append(
                f"Prioridad 2: Resolver {contradictions['total']} contradicciones detectadas"
            )

        # Acción 3: Basada en presupuesto
        if len(orchestrator_results.get("monetary", [])) == 0:
            actions.append(
                "Prioridad 3: Definir asignación presupuestal para programas"
            )
        ...
        if not teoria.get("is_valid", False):
            actions.append(
                "Prioridad 4: Desarrollar teoría de cambio explícita con validación causal"
            )
    ...
    @staticmethod
    def _export_results(
        results: Dict[str, Any], pdm_path: str, output_dir: Optional[str] = None
    ) -> Path:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Uso: python integrated_evaluation_system.py <ruta_pdm> [municipio] [departamento]"
        )
        print("\nEjemplo:")
