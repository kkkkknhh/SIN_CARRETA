...
def demo_causal_manager():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if model_A is None:
            print(
                f"  effect_A: Insufficient data ({logger.get_observation_count('effect_A')}/30)"
            )
        else:
        ...
        if model_B is not None:
            ...
            print(
                f"    β₁ = {summary['beta1_mean']:.3f} {summary['beta1_interval_95']}"
            )
...
def demo_periodic_analysis():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for plan_idx in range(60):
            ...
            feasibility2 = np.clip(feasibility2, 0, 1)
            logger.log_effect(
                "monetary_to_feasibility", monetary, feasibility2, f"plan_{plan_idx}"
            )
...
def demo_integration_with_policy_analysis():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if n_obs >= 30:
            ...
            print(
                f"  ⚠ Insufficient data for estimation (need {30 - n_obs} more observations)"
            )
