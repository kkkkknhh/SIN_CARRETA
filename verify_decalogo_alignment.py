...
def verify_questions(questions: List[Dict]) -> Tuple[List[str], Dict]:
    ...
    required_fields = {
        'id', 'dimension', 'question_no', 
        'point_code', 'point_title', 'prompt', 'hints'
    }
...
def verify_alignment(data: Dict) -> bool:
    ...
    if stats['total_questions'] == expected_total:
        print(f"  ✓ Total questions: {stats['total_questions']} (expected {expected_total})")
    else:
        error = f"Total questions: {stats['total_questions']} (expected {expected_total})"
