...
def check_scoring_references(file_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        has_scoring = any(
            ...
            for term in ["score", "scoring", "puntuación", "puntaje"]
        )
...
def main():
    ...
    try:
        ...
        print(
            f"{GREEN}✓{RESET} Loaded decalogo-industrial.latest.clean.json (v{decalogo['version']}, {decalogo['total']} questions)"
        )
        print(
            f"{GREEN}✓{RESET} Loaded dnp-standards.latest.clean.json (v{dnp['version']})"
        )
        print()
    except Exception as e:
    ...
    if output_dir.exists():
        ...
        print(
            f"{YELLOW}⚠{RESET} output/ directory missing (will be created on first run)"
        )
    print()
