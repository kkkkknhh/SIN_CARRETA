...
from deterministic_pipeline_validator import (
    ContractType,
    DeterministicPipelineValidator,
)
...
def manual_timing_breakdown():
    ...
    print("─" * 60)
    print(
        f"  Initialization:          {init_time_ms:8.4f} ms  ({init_time_ms / overall_time_ms * 100:5.1f}%)"
    )
    print(
        f"  Route selection test:    {route_time_ms:8.4f} ms  ({route_time_ms / overall_time_ms * 100:5.1f}%)"
    )
    print(
        f"  Tie-breaking test:       {tie_time_ms:8.4f} ms  ({tie_time_ms / overall_time_ms * 100:5.1f}%)"
    )
    print(
        f"  Result construction:     {result_time_ms:8.4f} ms  ({result_time_ms / overall_time_ms * 100:5.1f}%)"
    )
    print("─" * 60)
