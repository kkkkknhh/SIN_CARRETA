...
class Python310CompatibilityChecker:
    ...
    INCOMPATIBLE_PATTERNS = {
        'numpy': {
            'min_version': '1.21.0',
            'max_version': '1.25.0',
            'issues': ['Matrix API changes in 1.25+', 'Deprecated random number generation']
        },
        'scikit-learn': {
            'min_version': '1.0.0', 
            'max_version': '1.4.0',
    ...
    def check_numpy_compatibility(self) -> Dict[str, any]:
        ...
        try:
            ...
            if major < 1 or (major == 1 and minor < 21):
                ...
                compatibility_report['issues'].append(f"NumPy {np.__version__} lacks Python 3.10 support")
                compatibility_report['recommendations'].append("Upgrade to NumPy >= 1.21.0")
                
            if major > 1 or (major == 1 and minor >= 25):
                compatibility_report['issues'].append(f"NumPy {np.__version__} may have breaking changes")
                compatibility_report['recommendations'].append("Consider NumPy < 1.25.0 for stability")
            ...
            try:
                ...
                compatibility_report['python_310_compatible'] = False
                compatibility_report['issues'].append(f"NumPy core operations failed: {str(e)}")
    ...
    def run_compatibility_check(self) -> Dict[str, any]:
        ...
        summary = {
            ...
            'overall_compatible': python_ok and numpy_report['python_310_compatible'] and successful_imports >= len(self.CRITICAL_MODULES)
        }
