...
def safe_read_text_file(
    file_path: Union[str, Path], encodings: List[str] = None
) -> str:
    """
    ...
    for encoding in encodings:
        try:
            ...
            last_error = e
            logger.debug(
                "Failed to read %s with %s encoding: %s", file_path, encoding, e
            )
            continue
...
def clean_data_for_json(
    ...
    if isinstance(data, dict):
        return {
            key: clean_data_for_json(
                value, nan_replacement, inf_replacement, neg_inf_replacement
            )
        ...
        cleaned_list = [
            clean_data_for_json(
                item, nan_replacement, inf_replacement, neg_inf_replacement
            )
            for item in data
        ]
        ...
        if isinstance(data, np.floating):
            ...
            return clean_data_for_json(
                data.tolist(), nan_replacement, inf_replacement, neg_inf_replacement
            )
...
def safe_json_dump(
    ...
    **kwargs,
    ...
    default_params = {"ensure_ascii": False, "indent": 2, "allow_nan": False}

    # Update with any user-provided parameters
    default_params.update(kwargs)

    # Clean the data
    cleaned_data = clean_data_for_json(
        data, nan_replacement, inf_replacement, neg_inf_replacement
    )
...
def safe_json_write(
    ...
    **kwargs,
    ...
    with open(filepath, "w", encoding=encoding) as f:
        safe_json_dump(
            data, f, nan_replacement, inf_replacement, neg_inf_replacement, **kwargs
        )
