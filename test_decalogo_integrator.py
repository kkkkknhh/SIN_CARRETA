...
class TestSemanticExtractionThresholdEnforcement(unittest.TestCase):
    ...
    def test_irrelevant_segment_exclusion(self):
        ...
        irrelevant_texts = [
            ...
            if "geografía" in text.lower() or "montañas" in text.lower()
        ]
        self.assertEqual(
            len(irrelevant_texts), 0, "FAILED: Irrelevant segment not filtered"
        )
    ...
    def test_deterministic_ordering(self):
        ...
        self.assertEqual(results1, results2, "FAILED: Results not deterministic")
    ...
    def test_empty_segments_handling(self):
        ...
        self.assertEqual(
            len(results), 0, "FAILED: Should return empty list for empty input"
        )
...
class TestCausalGraphCycleDetection(unittest.TestCase):
    ...
    def test_cycle_detection(self):
        ...
        self.assertLess(
            result.acyclicity_pvalue, 0.95, "FAILED: p-value too high for cyclic graph"
        )
    ...
    def test_acyclic_graph_acceptance(self):
        ...
        self.assertGreaterEqual(
            result.acyclicity_pvalue, 0.95, "FAILED: p-value too low for valid DAG"
        )
    ...
    def test_confounder_identification(self):
        ...
        self.assertGreater(
            result.confounder_count, 0, "FAILED: No confounders identified"
        )
        self.assertIn("Z", result.confounders, "FAILED: Confounder Z not identified")

    def test_mediator_detection(self):
        """Test detection of mediator variables"""
        # Create graph with mediator: X -> M -> Y
        G = nx.DiGraph()
        G.add_edges_from([("X", "M", {"weight": 0.8}), ("M", "Y", {"weight": 0.7})])

        declared_mediators = {"M"}
        result = self.analyzer.analyze_dimension(G, "D1", declared_mediators)

        # REQUIRED: Mediator match
        self.assertTrue(result.mediator_match, "FAILED: Mediator not detected")
        self.assertEqual(
            len(result.mediator_mismatch), 0, "FAILED: Unexpected mediator mismatch"
        )
...
class TestBayesianIntegrationConflictDetection(unittest.TestCase):
    ...
    def test_consistent_evidence_integration(self):
        ...
        self.assertFalse(
            result["evidence_conflict_detected"], "FAILED: False conflict detection"
        )
    ...
    def test_credible_interval_generation(self):
        ...
        self.assertIn("credible_interval_95", result, "FAILED: No credible interval")
...
class TestDimensionCoverageCompleteness(unittest.TestCase):
    ...
    def test_all_300_questions_mapped(self):
        ...
        self.assertEqual(
            total_questions, 300, f"FAILED: Only {total_questions}/300 questions mapped"
        )
    ...
    def test_50_questions_per_dimension(self):
        ...
        for dim in ["D1", "D2", "D3", "D4", "D5", "D6"]:
            dim_questions = [
                q for q in extractor.mapping["questions"].keys() if q.startswith(dim)
            ]
...
class TestDeterministicExecution(unittest.TestCase):
    ...
    def test_results_determinism(self):
        ...
        self.assertEqual(
            hash1, hash2, "FAILED: Results not deterministic (hash mismatch)"
        )
...
class TestPerformanceMetrics(unittest.TestCase):
    ...
    def test_metrics_calculation(self):
        ...
        for metric in required_metrics:
            self.assertIn(metric, metrics, f"FAILED: Missing metric {metric}")
    ...
    def test_quality_gates_validation(self):
        ...
        for gate in required_gates:
            self.assertIn(gate, gates, f"FAILED: Missing quality gate {gate}")
    ...
    def test_coverage_metrics_accuracy(self):
        ...
        self.assertEqual(
            metrics["total_questions"], 300, "FAILED: Incorrect total questions count"
        )
        self.assertEqual(
            metrics["dimensions_fully_analyzed"],
            6,
...
class TestAntiPatternDetection(unittest.TestCase):
    ...
    def test_no_uniform_prior(self):
        ...
        self.assertNotEqual(
            (integrator.prior_alpha, integrator.prior_beta),
            (1.0, 1.0),
    ...
    def test_no_placeholder_implementations(self):
        ...
        results = extractor.extract_evidence(
            "test query", ["relevant segment about test query"], top_k=5
        )

        # Should return actual results, not placeholder
        self.assertIsInstance(results, list, "FAILED: Placeholder implementation")
        ...
        self.assertIsInstance(
            result.acyclicity_pvalue, float, "FAILED: Placeholder implementation"
        )
        self.assertGreaterEqual(
            result.acyclicity_pvalue, 0.0, "FAILED: Invalid p-value"
        )
...
def run_all_tests():
    ...
    suite.addTests(
        loader.loadTestsFromTestCase(TestSemanticExtractionThresholdEnforcement)
    )
    suite.addTests(loader.loadTestsFromTestCase(TestCausalGraphCycleDetection))
    suite.addTests(
        loader.loadTestsFromTestCase(TestBayesianIntegrationConflictDetection)
    )
