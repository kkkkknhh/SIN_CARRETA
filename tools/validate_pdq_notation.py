...
def validate_rubric(path: Path, result: ValidationResult) -> None:
    ...
    if total_keys != expected:
        result.add_warning(
            f"Rubric: Expected {expected} keys, found {total_keys}"
        )
...
def check_code_for_violations(repo_root: Path, result: ValidationResult) -> None:
    ...
    bad_patterns = [
        (r'question_id\s*=\s*["\']D\d+-Q\d+["\']', "D#-Q# without P# (should be P#-D#-Q#)"),
        (r'question_id\s*=\s*["\']P\d+-Q\d+["\']', "P#-Q# without D# (should be P#-D#-Q#)"),
    ...
    exclude_dirs = {".venv", "venv", ".git", "__pycache__", "node_modules", ".pytest_cache"}
