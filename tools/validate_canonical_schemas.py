...
def basic_validate_evidence(data: Dict[str, Any]) -> List[str]:
    ...
    if "question_unique_id" in data:
        if not basic_validate_pattern(data["question_unique_id"], r"^P(10|[1-9])-D[1-6]-Q[1-9][0-9]*$"):
            errors.append(f"Invalid question_unique_id format: {data['question_unique_id']}")
    
    if "content" in data:
        content = data["content"]
        if "rubric_key" in content:
            if not basic_validate_pattern(content["rubric_key"], r"^D[1-6]-Q[1-9][0-9]*$"):
...
def basic_validate_bundle(data: Dict[str, Any]) -> List[str]:
    ...
    if "questions" in data:
        for i, q in enumerate(data["questions"]):
            ...
            elif not basic_validate_pattern(q["question_unique_id"], r"^P(10|[1-9])-D[1-6]-Q[1-9][0-9]*$"):
                errors.append(f"Question {i}: invalid question_unique_id format: {q['question_unique_id']}")
            ...
            if "rubric_key" not in q:
                ...
                errors.append(f"Question {i}: invalid rubric_key format: {q['rubric_key']}")
...
def main():
    ...
    validations = [
        (repo_root / "config" / "RUBRIC_SCORING.json", schemas_dir / "rubric.schema.json"),
        (repo_root / "bundles" / "decalogo_bundle.json", schemas_dir / "decalogo_bundle.schema.json"),
    ]
