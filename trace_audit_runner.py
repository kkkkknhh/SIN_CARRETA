...
def generate_quick_report(stage_data, executed, errors):
    ...
    for i, stage in enumerate(expected, 1):
        ...
        if not exits:
            ...
            lines.append(f"{i:2d}. {stage:30s} {status_symbol} {status.upper():10s} ({duration:.3f}s)")
            lines.append(f"     Evidence: {evidence:3d} | Output: {output.get('type', 'unknown'):10s} size={output.get('size', 0)}")
    ...
    if errors:
        ...
        for err in errors:
            ...
            lines.append(f"  {err.get('error_type', 'Unknown')}: {err.get('error', 'No details')}")
            lines.append("")
