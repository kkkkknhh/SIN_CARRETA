...
from orchestrator.effects_logger import EffectsLogger, CausalEffectsManager
...
def demo_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, indicator_text in enumerate(indicators):
            ...
            print(f"    Components: baseline={score.has_baseline}, "
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for plan_idx in range(50):
            ...
            for _ in range(2):
                ...
                smart_score = (has_baseline + has_target + has_timeframe + 
                              has_quantitative) / 4.0
                
                # Log relationships manually (simulating what scorer would do)
                if has_baseline:
                    logger.log_effect("baseline_to_feasibility", 1.0, feasibility, plan_id)
                if has_target:
                    logger.log_effect("target_to_feasibility", 1.0, feasibility, plan_id)
                if has_timeframe:
        ...
        for effect_name in effects_to_analyze:
            if effect_name in logger.get_all_effects():
                ...
                if model:
                    ...
                    print(f"      95% CI: [{summary['beta1_interval_95'][0]:.3f}, "
                          f"{summary['beta1_interval_95'][1]:.3f}]")
                    print(f"      Significant: {summary['effect_significant']}")
                    print(f"      P(positive effect): {summary['prob_positive_effect']:.1%}")


if __name__ == "__main__":
    print("\n")
    print("╔" + "=" * 68 + "╗")
    print("║" + " " * 8 + "FEASIBILITY SCORER + CAUSAL EFFECTS INTEGRATION" + " " * 12 + "║")
