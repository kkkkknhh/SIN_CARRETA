...
class ValidationRunner:
    ...
    def run_check(
        self, name: str, command: List[str], timeout: int = 60
    ) -> Tuple[bool, str]:
    ...
    def check_python_version(self) -> bool:
        ...
        passed, output = self.run_check(
            "Python version", [sys.executable, str(script_path), "--strict"]
        )
    ...
    def check_conflicts(self) -> bool:
        ...
        if not script_path.exists():
            ...
            passed, output = self.run_check(
                "dependency conflicts", [sys.executable, "-m", "pip", "check"]
            )
        else:
            passed, output = self.run_check(
                "dependency conflicts", [sys.executable, str(script_path)]
            )
    ...
    def check_linting(self) -> bool:
        ...
        passed, output = self.run_check(
            "linting (flake8)", [sys.executable, "-m", "flake8", "."], timeout=120
        )
