...
class CodePathTracer:
        ...
        if event == "line":
            ...
            if "venv" not in filename and "site-packages" not in filename:
                self.function_calls.append(
                    {"function": funcname, "file": filename, "line": frame.f_lineno}
                )
    ...
    def get_coverage_for_file(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            ...
            coverage_percent = (
                (executed / total_executable * 100) if total_executable > 0 else 0
            )
...
def trace_test_flow(
    test_name: str, test_func, tracer: CodePathTracer
) -> Dict[str, Any]:
    ...
    def test_responsibility_detector():
        try:
            ...
            detector.detect_entities(
                "El Ministerio de Educación implementará el programa."
            )
            return True
        except:
            return False
...
def analyze_coverage_discrepancies(
    trace_results: List[Dict], expected_coverage: float = 65.0
):
    ...
    project_files = [
        f for f in project_files if f.name not in ["setup.py", "conftest.py"]
    ]
...
def main():
    ...
    for test_name, test_func in test_flows:
        ...
        print(
            f"    {status_symbol} {result['status']}: {result['lines_executed']} lines, "
