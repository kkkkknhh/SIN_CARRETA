...
class ModuleContribution:
    ...
    def __post_init__(self):
        ...
        if not 0 <= self.contribution_percentage <= 100:
            raise ValueError(
                f"Contribution must be 0-100, got {self.contribution_percentage}"
            )
...
class ModuleContributionMapper:
    ...
    def _load_mappings(self, path: Path):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.question_maps)} question mappings from {path}"
            )
    ...
    def _initialize_default_mappings(self):
        ...
        for dim in range(1, 7):  # D1-D6
            for point in range(1, 11):  # P1-P10
                for q_num in range(
                    1, 51
                ):  # Q1-Q50 (30 questions per D-P combo, +20 buffer)
    ...
    ) -> Optional[QuestionContributionMap]:
        """
...
if __name__ == "__main__":
    ...
    if mapping:
        ...
        print(
            f"Primary: {primary.module.value} ({primary.contribution_percentage:.1f}%)"
        )
