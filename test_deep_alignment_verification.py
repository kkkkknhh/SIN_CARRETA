...
def test_decalogo_loader():
    ...
    from decalogo_loader import get_decalogo_industrial, load_dnp_standards, ensure_aligned_templates
    
    print("Testing decalogo_loader...")
    
    industrial = get_decalogo_industrial()
    assert industrial['version'] == '1.0', f"Expected version 1.0, got {industrial['version']}"
    assert industrial['schema'] == 'decalogo_causal_questions_v1', f"Unexpected schema"
    assert industrial['total'] == 300, f"Expected 300 questions, got {industrial['total']}"
    assert len(industrial['questions']) == 300, f"Expected 300 actual questions, got {len(industrial['questions'])}"
    
    dnp = load_dnp_standards()
    assert dnp['version'] == '2.0_operational_integrated_complete', f"Unexpected DNP version"
    assert dnp['schema'] == 'estandar_instrucciones_evaluacion_pdm_300_criterios', f"Unexpected DNP schema"
...
def test_dimension_alignment():
    ...
    # Verify they match
    assert industrial_dims == dnp_dims, f"Dimension mismatch: industrial={industrial_dims}, dnp={dnp_dims}"
    
    expected_dims = {'D1', 'D2', 'D3', 'D4', 'D5', 'D6'}
    assert industrial_dims == expected_dims, f"Expected {expected_dims}, got {industrial_dims}"
...
def test_point_codes_alignment():
    ...
    assert point_codes == expected_points, f"Expected {expected_points}, got {point_codes}"
...
def test_question_id_format():
    ...
    assert len(invalid_ids) == 0, f"Found {len(invalid_ids)} invalid question IDs: {invalid_ids[:5]}"
...
def test_scoring_scale_alignment():
    ...
    rules = mapeo['reglas']
    expected_levels = ['0_AUSENTE', '1_INSUFICIENTE', '2_BASICO', '3_SATISFACTORIO', '4_AVANZADO']
