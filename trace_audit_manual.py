...
def analyze_orchestrator_code() -> Dict[str, Any]:
    ...
    if process_method_match:
        ...
        run_stage_calls = re.findall(
            r'self\._run_stage\(\s*PipelineStage\.(\w+)',
            method_body
        )
    ...
    if build_evidence_match:
        ...
        register_calls = re.findall(
            r'register_evidence\(\s*PipelineStage\.(\w+)',
            build_body
        )
...
def identify_code_issues(analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for stage_value, info in stage_analysis.items():
        ...
        if stage_num <= 12:
            ...
            if stage_value in detection_stages and not info['has_evidence_registration']:
                issues.append({
                    'stage': stage_value,
                    'stage_number': stage_num,
                    'severity': 'MEDIUM',
...
def generate_code_audit_report(analysis: Dict[str, Any], issues: List[Dict[str, Any]]) -> str:
    ...
    for stage_name in analysis['evidence_stages']:
        ...
        if stage_value:
            info = stage_analysis[stage_value]
            lines.append(f"  Stage {info['stage_number']:2d} ({stage_value:30s}) → Evidence registered")
        else:
