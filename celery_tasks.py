...
documents_processed_total = Counter(
    "documents_processed_total", "Total documents processed by workers", ["status"]
)
...
def process_document_task(
    self, job_id: str, document_path: str, document_index: int, _config: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        "Processing document %s for job %s: %s", document_index, job_id, document_path
    )
    ...
    try:
        ...
        results = pipeline.run_evaluation(
            ...
            output_dir=str(
                batch_manager.artifacts_base_dir
                / job_id
                / f"{job_id}_doc_{document_index}"
            ),
        )
        ...
        logger.info(
            "Successfully processed document %s for job %s in %.2fs",
            document_index,
            job_id,
            processing_time,
        )
        ...
        logger.error(
            "Failed to process document %s for job %s: %s",
            document_index,
            job_id,
            error_msg,
        )
            ...
            if results_file.exists():
                with open(results_file, "r", encoding="utf-8") as f:
                    ...
                    if "coverage_report" in doc_results:
                        coverage = (
                            doc_results["coverage_report"]
                            .get("summary", {})
                            .get("total_questions", 0)
                        )
                        aggregated_results["summary"]["total_questions_coverage"] += (
                            coverage
                        )

                    if "evidence_registry" in doc_results:
                        evidence_count = len(
                            doc_results["evidence_registry"].get("evidence", [])
                        )
                        aggregated_results["summary"]["total_evidence_items"] += (
                            evidence_count
                        )
