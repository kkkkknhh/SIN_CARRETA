...
def generate_executive_summary(data: Dict[str, Any]) -> str:
    ...
    stable_connections = sum(
        1 for c in connections.values() if c.get("stability", 0) >= 0.95
    )
    ...
    health_status = (
        "HEALTHY"
        if fail_count == 0 and gates_passed
        else "DEGRADED"
        if fail_count < 3
        else "CRITICAL"
    )
...
"""
    ...
    section += (
        f"**Status:** {'✓ ALL GATES PASSED' if gates_passed else '✗ GATES FAILED'}\n\n"
    )
...
"""
    ...
    for i, (severity, risk_type, location, description) in enumerate(risks, 1):
        severity_label = (
            "CRITICAL" if severity >= 80 else "HIGH" if severity >= 60 else "MEDIUM"
        )
...
def generate_top_fixes_section(
    risks: List[Tuple[int, str, str, str]], _nodes: Dict[str, Any]
) -> str:
...
"""
    ...
    if not risks:
        ...
        section += (
            "1. Implement response time budget alerts for early degradation detection\n"
        )
        section += (
            "2. Add capacity headroom monitoring (suggest scaling at 70% utilization)\n"
        )
        section += (
            "3. Set up synthetic monitoring to catch regressions pre-production\n"
        )
        return section
    ...
    for i, (severity, risk_type, location, description) in enumerate(risks, 1):
        ...
        if "Error Rate" in risk_type:
            ...
            section += (
                "- Consider batching/parallelization for independent operations\n"
            )
            ...
            section += "**Recommended Actions:**\n"
            section += (
                f"- Enforce strict schema validation at {location} input/output\n"
            )
