...
def test_dataclass_structure():
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and node.name == 'EvaluationResult':
            ...
            assert 'calibrated_score' in field_names, "EvaluationResult missing calibrated_score field"
            assert 'uncertainty' in field_names, "EvaluationResult missing uncertainty field"
        ...
        if isinstance(node, ast.ClassDef) and node.name == 'QuestionnaireEngine':
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and item.name == '__init__':
                    ...
                    assert 'reliability_calibrator' in init_source, "Missing reliability_calibrator initialization"
                    assert 'ReliabilityCalibrator' in init_source, "Missing ReliabilityCalibrator call"
...
def test_calibration_in_evaluation():
    ...
    assert 'expected_f1' in content or 'expected_reliability' in content, "Missing reliability computation"
...
def test_evidence_registration():
    ...
    assert 'calibration_evidence' in content or 'reliability_calibrator' in content, "Missing evidence registration"
