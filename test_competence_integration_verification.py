...
def verify_integration():
    ...
    results["per_question_validation"] = (
        "question_competence_issues" in content
        and "COMPETENCE VALIDATION PER QUESTION" in content
    )
    ...
    results["evidence_registration"] = (
        "competence_validation::" in content
        and "evidence_registry.register(" in content
    )

    # Check provenance metadata
    results["provenance_metadata"] = (
        '"validator_source": "CompetenceValidator"' in content
        and '"provenance"' in content
        and '"module": "pdm_contra.policy.competence"' in content
    )

    # Check error handling
    results["error_handling"] = (
        "except Exception" in content
        and "log_warning_with_text" in content
        and "⚠️ Error validando competencias" in content
        or "⚠️ Error en validación de competencias" in content
    )

    # Check results included in return value
    results["results_in_output"] = '"question_competence_validations":' in content
    ...
    results["proper_ordering"] = (
        contradiction_idx > 0
        and competence_per_q_idx > 0
        and scoring_idx > 0
        and contradiction_idx < competence_per_q_idx < scoring_idx
    )
    ...
    print(
        f"  - CompetenceValidator occurrences: {content.count('CompetenceValidator')}"
    )
    print(
        f"  - question_competence_validations references: {content.count('question_competence_validations')}"
    )
