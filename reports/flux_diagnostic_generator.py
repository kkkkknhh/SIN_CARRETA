...
class FluxDiagnosticGenerator:
    ...
    def validate_instrumentation_data(self) -> bool:
        ...
        for stage in stages:
            ...
            if missing:
                self.errors.append(
                    f"Stage '{stage.get('stage_name', 'unknown')}' missing fields: {missing}"
                )
                return False
            ...
            if not isinstance(stage.get("contract_checks"), dict):
                self.errors.append(
                    f"Stage '{stage['stage_name']}' contract_checks must be a dict"
                )
                return False

            if (
                "pass" not in stage["contract_checks"]
                or "fail" not in stage["contract_checks"]
            ):
                self.errors.append(
                    f"Stage '{stage['stage_name']}' contract_checks missing pass/fail"
                )
                return False
        ...
        for conn in connections:
            ...
            if missing:
                self.errors.append(
                    f"Connection '{conn.get('from_node', '?')}->{conn.get('to_node', '?')}' missing: {missing}"
                )
                return False
            ...
            if conn.get("suitability_verdict") not in valid_verdicts:
                self.errors.append(
                    f"Invalid suitability_verdict: {conn.get('suitability_verdict')}"
                )
                return False
    ...
    def generate_final_output_section(self) -> Dict[str, Any]:
        ...
        coverage_300 = self.diagnostic_data.get(
            "coverage_300", {"met": False, "actual_count": 0, "required_count": 300}
        )
        ...
        # Quality metrics
        quality_metrics = self.diagnostic_data.get(
            "quality_metrics",
            {
                "confidence_scores": {
                    "overall": 0.0,
                ...
                "evidence_completeness": {"percentage": 0.0, "missing_categories": []},
            },
        )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if not self.validate_instrumentation_data():
            raise ValueError(
                f"Instrumentation data validation failed: {'; '.join(self.errors)}"
            )
...
def get_rubric_exit_code() -> int:
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(rubric_script)], capture_output=True, timeout=30
        )
...
def main():
    ...
    try:
        ...
        print(
            json.dumps({"status": "validation_failed", "error": str(e)}),
            file=sys.stderr,
        )
        return 2
        ...
        print(
            json.dumps({"status": "error", "error": str(e), "type": type(e).__name__}),
            file=sys.stderr,
        )
        return 1
