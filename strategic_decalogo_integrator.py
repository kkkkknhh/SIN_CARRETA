...
class StructuredEvidence:
    ...
    def __post_init__(self):
        ...
        if self.evidence_type not in ['quantitative', 'qualitative']:
            raise ValueError(f"Evidence type must be 'quantitative' or 'qualitative', got {self.evidence_type}")
...
class SemanticExtractor:
    ...
    def extract_evidence(
        ...
        segment_embs = self.model.encode(segments, convert_to_tensor=True, show_progress_bar=False)
...
class CausalGraphAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(f"CausalGraphAnalyzer initialized (min_pvalue={self.min_pvalue_acyclicity})")
    ...
    def analyze_dimension(
        ...
        for source in graph.nodes():
            for target in graph.nodes():
                if source != target and nx.has_path(graph, source, target):
                    strength = self._calculate_causal_effect_backdoor(graph, source, target)
        ...
        return DimensionAnalysis(
            ...
            valid=True,
            acyclicity_pvalue=acyclicity_pvalue,
            avg_causal_strength=float(np.mean(list(causal_strengths.values()))) if causal_strengths else 0.0,
    ...
    def _test_acyclicity_bootstrap(self, G: nx.DiGraph, n_bootstrap: int = 1000) -> float:
        ...
        for _ in range(n_bootstrap):
            ...
            if edges:
                ...
                remove_edges = np.random.choice(len(edges), size=min(n_perturb, len(edges)), replace=False)
...
class BayesianEvidenceIntegrator:
    ...
    def __init__(self):
        ...
        self.prior_beta = 2.0
        logger.info(f"BayesianEvidenceIntegrator initialized (prior=Beta({self.prior_alpha},{self.prior_beta}))")
...
class DecalogoEvidenceExtractor:
    ...
    def __init__(
        ...
        logger.info(f"DecalogoEvidenceExtractor initialized with {len(self.mapping.get('questions', {}))} questions")
    ...
    def extract_for_question(self, question_id: str) -> List[StructuredEvidence]:
        ...
        for cev in canonical_evidence:
            ...
            structured.append(StructuredEvidence(
                question_id=question_id,
                dimension=dimension,
                evidence_type=evidence_type,
                raw_evidence=cev.content if isinstance(cev.content, dict) else {'value': cev.content},
                processed_content=cev.content if isinstance(cev.content, dict) else {'score': 0.5},
...
class StrategicDecalogoIntegrator:
    ...
    def _calculate_performance_metrics(
        ...
        questions_with_evidence = sum(
            sum(1 for q in dim_data['question_evidence'].values() if q['evidence_count'] > 0)
            for dim_data in results.values()
        )
        ...
        for dim_data in results.values():
            for q_result in dim_data['question_evidence'].values():
                ...
                ci_width = bayesian['credible_interval_95'][1] - bayesian['credible_interval_95'][0]
        ...
        return {
            ...
            'avg_evidence_confidence': float(np.mean(avg_confidence)) if avg_confidence else 0.0,
            'pct_questions_high_confidence': float(sum(1 for c in avg_confidence if c > 0.7) / len(avg_confidence)) if avg_confidence else 0.0,
            'pct_evidence_conflicts': float(len(conflicts) / total_questions) if total_questions > 0 else 0.0,
    ...
    def _validate_quality_gates(
        ...
        return {
            ...
            'min_evidence_coverage': metrics['questions_with_evidence'] >= 240,  # 80% minimum
