...
try:
    ...
    logger.warning(
        "spaCy not available. Using pattern-based fallback for entity detection."
    )
    ...
    **EVIDENCE REGISTRY**: Automatically registers findings with applicable questions.
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if args:
            if len(args) > 1:
                raise TypeError(
                    "ResponsibilityDetector accepts at most one positional argument"
                )
            model_loader = args[0]
        ...
        self.compiled_government_patterns = [
            re.compile(p) for p in self.government_patterns
        ]
        self.compiled_position_patterns = [
            re.compile(p) for p in self.position_patterns
        ]
        self.compiled_institution_patterns = [
            re.compile(p) for p in self.institution_patterns
        ]
    ...
    def _extract_spacy_entities(
        ...
        for ent in doc.ents:
            ...
            if ent.label_ in ["ORG", "PERSON"]:
                entity_type = (
                    EntityType.INSTITUTION if ent.label_ == "ORG" else EntityType.PERSON
                )
    ...
    def _create_entity_from_match(
        ...
        if len(entity_text.split()) > 2:
            confidence += 0.1
        if boost_keywords and any(
            keyword in entity_text.lower() for keyword in boost_keywords
        ):
            confidence += 0.1
    ...
    @staticmethod
    def _merge_entities(
        entities1: List[ResponsibilityEntity], entities2: List[ResponsibilityEntity]
    ) -> List[ResponsibilityEntity]:
        """
        ...
        for current in all_entities[1:]:
            ...
            if current.start_pos < previous.end_pos:
                ...
                if current.confidence > previous.confidence:
                    ...
                    and current.entity_type == EntityType.GOVERNMENT
                ):
    ...
    def _detect_entity_roles(
        self, entities: List[ResponsibilityEntity], _text: str
    ) -> List[ResponsibilityEntity]:
        """
        ...
        for entity in entities:
            ...
            for pattern in self.compiled_role_patterns:
                ...
                for match in matches:
                    ...
                    entity_pos_in_context = entity.start_pos - (
            ...
            if entity.entity_type == EntityType.POSITION:
                ...
                for other in entities:
                    if (
                        other.entity_type
                        in [EntityType.GOVERNMENT, EntityType.INSTITUTION]
                        and entity.text.lower() in other.context.lower()
                    ):
    ...
    @staticmethod
    def evaluate_responsibility_clarity(
        ...
        position_count = sum(
            1 for e in entities if e.entity_type == EntityType.POSITION
        )
        ...
        if clarity_score >= 0.8:
            ...
            recommendation = (
                "Las responsabilidades institucionales están claramente definidas."
            )
    ...
    def analyze_document(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "answer_de1_q2": evaluation["categorization"]
            in ["well_defined", "partially_defined"],
...
def create_responsibility_detector(
