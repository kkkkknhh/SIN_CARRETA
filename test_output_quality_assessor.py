...
class TestOutputQualityAssessor:
    ...
    def test_evidence_distribution_metrics(tmp_path):
        ...
        assert (
            results["metrics"]["evidence_distribution"]["questions_with_zero_evidence"]
            == 1
        )
    ...
    def test_rationale_completeness_metrics(tmp_path):
        ...
        assert (
            results["metrics"]["rationale_completeness"]["questions_with_rationale"]
            == 2
        )
        assert results["metrics"]["rationale_completeness"]["total_questions"] == 4
        assert (
            results["metrics"]["rationale_completeness"]["completeness_percentage"]
            == 50.0
        )
    ...
    def test_overall_pass_determination(tmp_path):
        ...
        with open(answers_file, "w") as f:
            json.dump(
                {"question_answers": [{"question_id": f"Q{i}"} for i in range(300)]}, f
            )
