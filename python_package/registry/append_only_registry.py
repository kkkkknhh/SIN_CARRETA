...
class AppendOnlyRegistry:
    ...
    def verify_chain(self) -> tuple[bool, Optional[str]]:
        ...
        if first_entry.prev_hash != self.GENESIS_HASH:
            return False, f"First entry prev_hash should be genesis hash, got {first_entry.prev_hash}"
        
        # Verify each entry's hash
        for i, entry in enumerate(self.entries):
            # Check sequence number
            if entry.sequence_number != i:
                return False, f"Entry {i}: sequence_number mismatch (expected {i}, got {entry.sequence_number})"
            
            # Recompute hash
            expected_hash = self._compute_entry_hash(entry.prev_hash, entry.packet)
            if entry.entry_hash != expected_hash:
                return False, f"Entry {i}: hash mismatch (expected {expected_hash}, got {entry.entry_hash})"
            
            # Check chain linkage (except for first entry)
            if i > 0:
                prev_entry = self.entries[i - 1]
                if entry.prev_hash != prev_entry.entry_hash:
                    return False, f"Entry {i}: chain broken (prev_hash doesn't match previous entry_hash)"
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("       python -m python_package.registry.append_only_registry <registry_path>")
        sys.exit(1)
